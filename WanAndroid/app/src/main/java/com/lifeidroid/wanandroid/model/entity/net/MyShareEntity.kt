 ...
 *     time   : 2022/11/03
 *     desc   :10.3 自己的分享的文章列表
 *     version: 1.0
 * </pre>
 */
@JsonClass(generateAdapter = true)
data class MyShareEntity(
    @Json(name = "coinInfo")
    var coinInfo: CoinInfo? = CoinInfo(),
    @Json(name = "shareArticles")
    var shareArticles: ShareArticles? = ShareArticles()
) {
    @JsonClass(generateAdapter = true)
    data class CoinInfo(
        @Json(name = "coinCount")
    ...
    data class ShareArticles(
        ...
        var curPage: Int? = 0,
        @Json(name = "datas")
        var datas: List<ArticleEntity.Data>? = listOf(),
        @Json(name = "offset")
        var offset: Int? = 0,
        @Json(name = "over")
